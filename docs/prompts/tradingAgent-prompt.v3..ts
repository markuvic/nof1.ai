...
export function getStrategyParams(strategy: TradingStrategy): StrategyParams {
  ...
  // 超短线策略：50%-75% 的最大杠杆（降低杠杆以控制极短周期风险）
  const ultraShortLevMin = Math.min(maxLeverage, Math.max(3, Math.ceil(maxLeverage * 0.5)));
  const ultraShortLevMax = Math.min(maxLeverage, Math.max(5, Math.ceil(maxLeverage * 0.75)));
  const ultraShortLevNormal = ultraShortLevMin;
  const ultraShortLevGood = Math.min(ultraShortLevMax, Math.max(ultraShortLevMin, Math.ceil((ultraShortLevMin + ultraShortLevMax) / 2)));
  const ultraShortLevStrong = ultraShortLevMax;

  // 波段趋势策略：20%-50% 的最大杠杆（中低杠杆，支持更长持仓）
  const swingTrendLevMin = Math.min(maxLeverage, Math.max(2, Math.ceil(maxLeverage * 0.2)));
  const swingTrendLevMax = Math.min(maxLeverage, Math.max(5, Math.ceil(maxLeverage * 0.5)));
  const swingTrendLevNormal = swingTrendLevMin;
  const swingTrendLevGood = Math.min(swingTrendLevMax, Math.max(swingTrendLevMin, Math.ceil((swingTrendLevMin + swingTrendLevMax) / 2)));
  const swingTrendLevStrong = swingTrendLevMax;
  ...
  const strategyConfigs: Record<TradingStrategy, StrategyParams> = {
    ...
    "ultra-short": {
      ...
      tradingStyle: "超短线交易，5分钟执行周期，快速捕捉短期波动，严格执行2%周期锁利和30分钟盈利平仓规则",
    },
    "swing-trend": {
      name: "波段趋势",
      description: "中长线波段交易，20分钟执行，捕捉中期趋势，适合稳健成长",
      leverageMin: swingTrendLevMin,
      leverageMax: swingTrendLevMax,
      leverageRecommend: {
        normal: `${swingTrendLevNormal}倍`,
        good: `${swingTrendLevGood}倍`,
        strong: `${swingTrendLevStrong}倍`,
      },
      positionSizeMin: 12,
      positionSizeMax: 20,
      positionSizeRecommend: {
      ...
      volatilityAdjustment: {
        highVolatility: { leverageFactor: 0.5, positionFactor: 0.6 },
        normalVolatility: { leverageFactor: 1.0, positionFactor: 1.0 },
        lowVolatility: { leverageFactor: 1.2, positionFactor: 1.1 },
      },
      entryCondition: "至少3个以上时间框架信号强烈一致，优先15分钟-4小时级别，耐心等待趋势形成",
      riskTolerance: "单笔交易风险控制在12-20%之间，注重趋势质量而非交易频率",
      tradingStyle: "波段趋势交易，20分钟执行周期，持仓时间可达数天，通过分批止盈和移动止盈让利润充分奔跑",
    },
  };
...
${strategy === 'ultra-short' ? '策略提示：当前使用 5 分钟执行周期，请重点分析 1-5 分钟级别信号，并严格落实 2% 周期锁利与 30 分钟盈利平仓规则。' : ''}
${strategy === 'swing-trend' ? '策略提示：当前使用 20 分钟执行周期，请优先分析 15 分钟至 4 小时级别的趋势共振，耐心等待高质量入场并让利润充分奔跑。' : ''}
...
- **风控策略（系统硬性底线 + AI战术灵活性）**：
  ...
  * 持仓时间 ≥ 36小时：系统强制平仓（释放资金）
  ...
  (1) 止损策略（⚠️ 必须严格遵守，这是保护本金的生命线）：
     * 策略止损线（必须执行，不可突破）：
       - ${params.leverageMin}-${Math.floor((params.leverageMin + params.leverageMax) / 2)}倍杠杆：止损 ${params.stopLoss.low}%（必须执行）
       - ${Math.floor((params.leverageMin + params.leverageMax) / 2)}-${Math.ceil((params.leverageMin + params.leverageMax) * 0.75)}倍杠杆：止损 ${params.stopLoss.mid}%（必须执行）
       - ${Math.ceil((params.leverageMin + params.leverageMax) * 0.75)}-${params.leverageMax}倍杠杆：止损 ${params.stopLoss.high}%（必须执行）
     * 仅可根据关键支撑/压力位微调±1%以内（超过即视为违规）
     * ⚠️ 触及止损线必须立即平仓，不要等待反弹或寻找理由
     * 说明：pnl_percent已包含杠杆效应，直接比较即可
  
  (2) 移动止盈策略（灵活锁住利润，依据行情动态调整）：
     ...
     * 灵活判断：遇到明显阻力位、趋势减弱或出现反转信号时，可提前止盈或分批落袋
     * 即使盈利只有2%-3%，只要趋势不再占优，也可以果断止盈，避免利润回吐
     * 出现反转信号必须立即止盈，不要死守更高目标
     * 说明：这些阈值提供参考锚点，最终以实时行情为准
...
2. 现有持仓管理（优先于开新仓）：
   ...
   a) 止损决策（⚠️ 必须严格执行）：
      ...
      - 仅可根据关键支撑位在 ±1% 内进行精细调整，严禁突破止损线
      - ⚠️ 触及止损线必须立即调用 closePosition 平仓，不要犹豫或等待反弹

   b) 移动止盈决策：
      - 检查是否达到移动止盈触发点（+${params.trailingStop.level1.trigger}%/+${params.trailingStop.level2.trigger}%/+${params.trailingStop.level3.trigger}%）
      - 如果达到，评估是否需要移动止损线保护利润，或提前分批止盈
      - 若盈利明显回落、趋势减弱或到达重要阻力位，可立即执行 closePosition 落袋为安
   ...
   c) 分批止盈决策：
      ...
      - 评估趋势强度，决定是否分批止盈
      - 如果决定分批止盈，再调用 closePosition 的 percentage 参数部分平仓；盈利较小但趋势减弱时，也可以适当分批
...
同周期保护机制（系统提示）：
- 系统会自动拒绝平掉持仓时间少于 ${intervalMinutes} 分钟的仓位（防止同一周期内频繁进出导致高额手续费）
- 如果尝试平仓被拒绝，说明该仓位太新，请等待下一个周期再评估
- 这是为保护交易成本而设定的硬性风控，请在决策中考虑这一规则
