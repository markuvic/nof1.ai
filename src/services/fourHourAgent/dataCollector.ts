...
export type FourHourTimeframe = "30m" | "4h" | "1d";

const TIMEFRAME_CONFIG: Record<
	FourHourTimeframe,
	{ interval: string; limit: number }
> = {
	"30m": { interval: "30m", limit: 60 },
...
export interface FourHourMarketDataset {
	symbols: string[];
	k: Record<string, Record<FourHourTimeframe, PromptCandle[]>>;
	indicators: Record<
		string,
		Record<FourHourTimeframe, TimeframeIndicators>
	>;
...
export async function collectFourHourMarketDataset(
	symbols: string[],
): Promise<FourHourMarketDataset> {
	const exchangeClient = createExchangeClient();
	const dataset: FourHourMarketDataset = {
		symbols: [],
		k: {},
		indicators: {},
		fundingRates: {},
...
function deriveMarketEnvironment(
	dataset: Omit<FourHourMarketDataset, "marketEnvironment">,
	...
	for (const symbol of dataset.symbols) {
		const k30 = dataset.k[symbol]?.["30m"] ?? [];
		const volatilityCandidate =
			k30.length > 1 ? averageAbsoluteReturn(k30) * 100 : undefined;
		if (
