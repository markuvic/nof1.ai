...
import type {
	HybridContext,
	HybridSymbolSnapshot,
} from "../services/hybridContext";
...
function formatSymbolSnapshot(
	symbol: string,
	snapshot: HybridSymbolSnapshot,
	hybridContext: HybridContext,
): string {
	const lines: string[] = [];
	lines.push(`### ${symbol}`);
	...
	lines.push(
		`资金费率 ${formatNumber(snapshot.fundingRate, 6)} (${snapshot.fundingDirection === "positive" ? "多头付费" : snapshot.fundingDirection === "negative" ? "空头付费" : "中性"})`,
	);
	lines.push(
		`成交量 当前 ${formatNumber(snapshot.volume.current, 1)} vs 平滑 ${formatNumber(snapshot.volume.average, 1)}`,
	);
	lines.push(
		`ATR14 ${formatNumber(snapshot.atr.value, 3)} (${formatNumber(snapshot.atr.percent, 2)}%) ${describeDirection(snapshot.atr.direction)} Δ${formatNumber(snapshot.atr.delta, 3)}`,
	);
	lines.push(
		`盘口压差：买盘 ${formatNumber(snapshot.orderBookPressure.bidVolume, 1)} / 卖盘 ${formatNumber(snapshot.orderBookPressure.askVolume, 1)}，差值 ${formatNumber(snapshot.orderBookPressure.delta, 1)}，比例 ${formatNumber(snapshot.orderBookPressure.ratio, 2)}`,
	);
	lines.push(
...
function formatAccountSection(accountInfo: any): string {
	...
	const available = Number(
		accountInfo?.availableBalance ?? accountInfo?.available ?? 0,
	);
	...
	const drawdownFromInitial =
		initial > 0 ? ((initial - total) / initial) * 100 : 0;
	lines.push(
		`当前账户价值：${formatNumber(total, 2)} USDT（可用 ${formatNumber(available, 2)}，未实现盈亏 ${formatNumber(unrealized, 2)}）`,
	);
	lines.push(
		`初始 ${formatNumber(initial, 2)} USDT / 峰值 ${formatNumber(peak, 2)} USDT`,
	);
	lines.push(
		`回撤：从峰值 ${formatNumber(drawdownFromPeak, 2)}%，从初始 ${formatNumber(drawdownFromInitial, 2)}%`,
	);
	lines.push(
		`账户收益率：${formatNumber(Number(accountInfo?.returnPercent ?? 0), 2)}%，Sharpe ${formatNumber(Number(accountInfo?.sharpeRatio ?? 0), 3)}`,
	);
	return lines.join("\n");
}
...
function formatFeedback(tradeHistory: any[] = []): string {
	...
	const pnlText =
		last.pnl !== null && last.pnl !== undefined
...
export function generateHybridPrompt(data: HybridPromptInput): string {
	...
	const triggerNote =
		triggerReason === "market-pulse"
			? (pulseSummary ??
				"⚡ 市场脉冲触发：请立即聚焦 BTC 及主导资产的异常波动，并评估是否需要提前执行策略。")
			: "本轮为常规调度执行。";
