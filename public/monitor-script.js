...
class TradingMonitor {
    ...
    updateStatValue(elementId, text, tone) {
    ...
    async loadStatsData() {
        try {
            ...
            const profitFactor = Number.isFinite(data.profitFactor)
                ? Number(data.profitFactor)
                : null;
            const lossTrades = Number(data.lossTrades ?? 0);
            const totalPnlValue = Number(data.totalPnl ?? 0);
            const averageWinValue = Number.isFinite(Number(data.averageWin))
                ? Number(data.averageWin)
                : 0;
            const averageLossValue = Number.isFinite(Number(data.averageLoss))
                ? Number(data.averageLoss)
                : 0;
            ...
            this.updateStatValue(
                'stats-total-trades',
                Number.isFinite(totalTrades) ? totalTrades.toString() : '--'
            );
            this.updateStatValue(
                'stats-win-rate',
                this.formatPercent(winRate),
            ...
            this.updateStatValue('stats-profit-factor', profitFactorText, profitFactorTone);

            this.updateStatValue(
                'stats-total-pnl',
                this.formatSignedNumber(totalPnlValue, 2),
                totalPnlValue > 0 ? 'positive' : totalPnlValue < 0 ? 'negative' : undefined
            );
            this.updateStatValue(
                'stats-average-win',
                this.formatSignedNumber(averageWinValue, 2),
                averageWinValue > 0 ? 'positive' : averageWinValue < 0 ? 'negative' : undefined
            );
            this.updateStatValue(
                'stats-average-loss',
                this.formatSignedNumber(averageLossValue, 2),
                averageLossValue < 0 ? 'negative' : averageLossValue > 0 ? 'positive' : undefined
            );
            const totalFeeText = Number.isFinite(totalFeeValue)
                ? `-${totalFeeValue.toFixed(2)}`
                : '--';
            const averageFeeText = Number.isFinite(averageFeeValue)
                ? `-${averageFeeValue.toFixed(4)}`
                : '--';
            this.updateStatValue(
                'stats-total-fee',
                totalFeeText,
                totalFeeValue > 0 ? 'negative' : undefined
            );
            this.updateStatValue(
                'stats-average-fee',
                averageFeeText,
                averageFeeValue > 0 ? 'negative' : undefined
            );
        } catch (error) {
    ...
    async loadQuantReports() {
        ...
        try {
            ...
            container.innerHTML = reports.map((report) => {
                ...
                const horizon = report.decision?.forecastHorizon || '未知';
                    ...
                    <div class="quant-report-card">
                        ...
                        <div class="quant-report-body">
                            <div>
                                ...
                                <div class="value">${horizon} · R/R ${rr}</div>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <div class="value">${report.indicatorReport || ''}</div>
                            </div>
                            <div>
                                <div class="label">形态</div>
                                <div class="value">${report.patternReport || ''}</div>
                            </div>
                            <div>
                                <div class="label">趋势</div>
                                <div class="value">${report.trendReport || ''}</div>
                            </div>
                        </div>
                    </div>
                `;
